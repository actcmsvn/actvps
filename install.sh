...
options=("PHP 7.4" "PHP 7.3" "PHP 7.2" "PHP 7.1" "PHP 5.6")
PS3="$prompt"
...
read admin_port
if [ "$admin_port" == "" ] || [ $admin_port == "2020" ] || [ $admin_port -lt 2000 ] || [ $admin_port -gt 9999 ] || [ $(lsof -i -P | grep ":$admin_port " | wc -l) != "0" ]; then
	admin_port=$(date +'%Y')
...
enabled  = true
filter   = sshd
action   = iptables[name=SSH, port=2020, protocol=tcp]
...
printf "=========================================================================\n"
printf "***Luu y: Port dang nhap SSH da duoc doi tu 22 sang 2020 de bao mat VPS\n"
printf "=========================================================================\n"
